...
function outerFunction(array) {
  ...
  return function (gst) {
    ...
    let payablePrice = totalPrice * (1 + gst / 100);
...
let outerFunction1 = (array) => {
  let totalPrice = 0;
  array.forEach((each) => {
    totalPrice += each;
  });
  return (gst) => {
    let payablePrice = totalPrice * (gst / 100) + totalPrice;
    return "$" + payablePrice.toFixed(2);
  };
};

console.log("Total price + GST =", outerFunction1(priceList)(gstPercent));
